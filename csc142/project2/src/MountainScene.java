...
public class MountainScene extends GWindowEventAdapter {
	...
	public void timerExpired(GWindowEvent we) {
		...
		int mo=animationCounter%10;
		if (mo==0||mo==1||mo==2||mo==3||mo==4||mo==5){
			this.bird1.angryy();
			this.bird2.angryy();
		}else{
		this.bird1.stupidd();
		...
		this.bird1.rotateNose();
		this.bird2.rotateNose();
	...
	private void addGraphicsElements() {
		...
		// this.myElement = new ???;
		this.bird1=new Bird (350,300,1.1,this.window);
		this.bird2=new Bird (50,80,1,this.window);
		// Code to do the animation
				this.window.addEventHandler(this);
				this.window.startTimerEvents(1000); // redraw every 150 ms
