...
public class Controler implements MouseListener {
	...
	public Controler(DrawModel model, JPanel jpanel) {
		...
		one = new JRadioButton("11111");
		one.setSelected(true);
		one.addActionListener(al);
		group.add(one);
		jpanel.add(one);

		two = new JRadioButton("2222");
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.isMetaDown()) {
			...
			if (height/2<=1){
				...
				JOptionPane.showMessageDialog(null, "height less than 1 will kill my computer","information",JOptionPane.INFORMATION_MESSAGE);
