...
public class Demo9_3 extends JFrame implements ActionListener{
	...
	public Demo9_3()
	{
		...
		this.add(mp);//因为中间是默认的，所以不用带什么参数了
		this.add(jb2, BorderLayout.SOUTH);
		
		Cat myCat1=new Cat();
		
		// 注册监听,把addActionListener设为myCat1,就把监听者设为Cat 的实例
		jb1.addActionListener(myCat1);
		jb2.addActionListener(myCat1);
		
		//注册监听, 表示Demo9_3  所创建的所有对象都监听此对象
		jb1.addActionListener(this);
		//指定action命令,
		jb1.setActionCommand("aa");  
		// 被监听者发出的字符信息，可以传给事情处理的方法
		//事情处理方法根据传来的信息，判断 区分是哪个按钮发出的信息
		
		jb2.addActionListener(this);   // this 表示Demo9_3  所创建的所有对象都监听此对象
